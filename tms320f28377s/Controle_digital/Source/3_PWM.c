...
void PWM (void) {
	...
	EPwm1Regs.TBPRD = 4000; 					// Period
	EPwm1Regs.TBPHS.bit.TBPHS = 0;				// Delay between ePWM's
	EPwm1Regs.TBCTL.bit.PHSEN = TB_DISABLE;		// Enable TBPHS
	EPwm1Regs.TBCTL.bit.CTRMODE = TB_COUNT_UP;	// Counter Mode

	EPwm1Regs.CMPCTL.bit.SHDWAMODE = 0;	// Shadow
	EPwm1Regs.CMPCTL.bit.SHDWBMODE = 0;	// Shadow
	EPwm1Regs.CMPCTL.bit.LOADAMODE = 2;	// load on ZRO and PRD
	EPwm1Regs.CMPCTL.bit.LOADBMODE = 2;	// load on ZRO and PRD

	EPwm1Regs.CMPA.bit.CMPA = 500;	// Compare A
	EPwm1Regs.CMPB.bit.CMPB = 0; 	// Compare B

	EPwm1Regs.AQCTLA.bit.ZRO = AQ_CLEAR;	// Saída ePWM2A
	EPwm1Regs.AQCTLA.bit.CAU = AQ_CLEAR;

	EPwm1Regs.AQCTLB.bit.ZRO = AQ_CLEAR;	// Saída ePWM2B
...
/*	EPwm2Regs.DBCTL.bit.IN_MODE = DBA_ALL;
	...
	EPwm1Regs.ETSEL.bit.SOCASEL = 1;	// Determine when a EPWMxSOCA pulse will be generated. (1 -> When CTR = ZERO)
	EPwm1Regs.ETPS.bit.SOCAPRD = 1;		// Generate pulse on 1st event.

	EPwm1Regs.TZSEL.bit.OSHT1 = TZ_ENABLE;	// Enable One Shot 1 Event for ePWM2 Trip Zone
	EPwm1Regs.TZCTL.bit.TZA = TZ_FORCE_LO;	// EPWM2A will be forced low on a trip event
	EPwm1Regs.TZCTL.bit.TZB = TZ_FORCE_LO;	// EPWM2B will be forced low on a trip event
